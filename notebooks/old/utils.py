...
def volume_LR(ds):        
    ...
    VOL = (dg.dz * dg.DXT* dg.DYT).compute()
    ds['VOL']=VOL.compute()
...
def volume(ds):
    ...
    return ds
#=====================================================================================
#======== Horizontal Operators on the POP grid  - modified from Anna-Lena/POP-tools/XGCM ==
#=====================================================================================
...
def rm_season(ds):
    da=ds.groupby('time.month')-ds.groupby('time.month').mean()
    return da
...
def anomaly(da):
    anom = da.groupby('time.month') - climatology(da)
    return anom
...
# imageio.mimsave('animation/movie.gif', images)